...
<head>
    ...
    <div class="content">
        ...
        <form method="post" action="{{route('categories_edit')}}" enctype="multipart/form-data">
            <input name="_token" type="hidden" value="{{ csrf_token() }}">
            <input name="category_id" type="hidden" value="{{$its_category}}">
        <!-- CKEditor default -->
        <div class="panel panel-flat">
            <div class="form-button">
                <button type="submit" class="btn bg-teal-400">{{$languages[0][0]['upload_button_name']}}</button>
            </div>
            <div class="select-category">
            <span>Выберите категорию:</span>
            <select class="selectpicker" name="parent_category">
                <option selected>{{$parent_category}}</option>
                                ...
                                <div class="panel-heading">
                                    <div class="form-inline">
                                        <div class="form-group">
                                            <label>Name*</label>
                                            <input name="name[0]" type="text" id="name[0]" value="{{$categories[0]->name}}" placeholder="{{$languages[0][0]['name_placeholder']}}">
                                        </div>
                                        <div class="form-group ">
                                            <label>URL*</label>
                                            <input name="url[0]" type="text" id="url[0]" value="{{$categories[0]->url}}" placeholder="{{$languages[0][0]['url_placeholder']}}">
                                        </div>
                                        <div class="form-group">
                                            <label>Title*</label>
                                            <input name="title[0]" type="text" id="title[0]" value="{{$categories[0]->title}}" placeholder="{{$languages[0][0]['title_placeholder']}}">
                                        </div>
                                        <div class="form-group">
                                            <label>Description*</label>
                                            <input name="description[0]" type="text" id="description[0]" value="{{$categories[0]->description}}"
                            ...
                            <div role="tabpanel" class="tab-pane fade" id="{{$languages[1][0]['language_url']}}">
                                    ...
                                    <div class="panel-heading">
                                        <div class="form-inline">
                                            <div class="form-group">
                                                <label>Name*</label>
                                                <input name="name[1]" type="text" id="name[1]" value="{{$categories[1]->name}}" placeholder="{{$languages[1][0]['name_placeholder']}}">
                                            </div>
                                            <div class="form-group ">
                                                <label>URL*</label>
                                                <input name="url[1]" type="text" id="url[1]" value="{{$categories[1]->url}}" placeholder="{{$languages[1][0]['url_placeholder']}}">
                                            </div>
                                            <div class="form-group">
                                                <label>Title*</label>
                                                <input name="title[1]" type="text" id="title[1]" value="{{$categories[1]->title}}" placeholder="{{$languages[1][0]['title_placeholder']}}">
                                            </div>
                                            <div class="form-group">
                                                <label>Description*</label>
                                                <input name="description[1]" type="text" id="description[1]" value="{{$categories[1]->description}}"
